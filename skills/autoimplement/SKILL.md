---
name: autoimplement
description: Run claude-basics/auto_implement_issues.sh against eligible GitHub issues. Handles the Windows Git Bash process-substitution bug by iterating per-issue, and cleans up stale research worktrees. Use when the user types /autoimplement.
user_invocable: true
---
...
The script's default (no-arg) path uses `<(...)` process substitution, which is broken on Windows Git Bash (`jq: Could not open /proc/<pid>/fd/<n>`). **Always invoke with `--issue N`** — even for multi-issue runs, iterate per-ID. Harmless on Linux/macOS.
